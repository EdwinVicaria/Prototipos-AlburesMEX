                ...
                @if (Route::has('login'))
                    @auth
                        @if(Auth()->user()-> id == $alburs->user_id) 
                            ...
                            <button wire:click="abrirEdit({{$alburs->id}})" class="border border-2 border-black rounded-lg p-2 bg-green-700">Editar</button>
                        @endif
                    @else
        ...
        <div class="grid grid-cols-3 gap-4">
            ...
            <div class="absolute inset-0 max-w-md">
                <form wire:submit.prevent="guardarAlbur" class=" p-4 text-white rounded-lg bg-sky-900 bg-opacity-50">
                    ...
                    <div>
                        <button type="submit" class="border outline-double outline-3 outline-offset-2 outline-white p-2 bg-sky-900 rounded-lg  w-32 h-10 animate-bounce">Enviar registro</button>
                        <button wire:click="cerrarEdit()" class="mb-2 md:mb-0 bg-white px-5 py-2 text-sm shadow-sm font-medium tracking-wider border text-gray-600 rounded-full hover:shadow-lg hover:bg-gray-100">
                            Cancelar
                        </button>
                    </div>
